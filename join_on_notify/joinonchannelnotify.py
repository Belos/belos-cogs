...
Message Examples:
    ...
    async def status(self, ctx):
        ...
        e.add_field(name="Interview Channel:", value="#" + self.bot.get_channel(db[server.id]["Channel"]).name if self.bot.get_channel(db[server.id]["Channel"]) else None, inline=True)
    ...
    async def joinmessage(self, ctx, *, message : str):
        ...
        db = fileIO(self.load, "load")
        if ctx.message.server.id not in db:
            await self.bot.say(":x: **Please set the channel you want me to send interview and leaving messages to with `Interview channel #channel_name` then you may proceed to setting this message..**")
            return
    ...
    async def embed(self, ctx):
        ...
        db = fileIO(self.load, "load")
        if not server.id in db:
            await self.bot.say("**Please set the channel you want me to send interview and leaving messages to with `interview channel #channel_name` then you may proceed to setting this message.**")
            return
    ...
    async def disable(self, ctx):
        ...
        db = fileIO(self.load, "load")
        if not server.id in db:
            await self.bot.say("Interview was never enabled on this server. :face_palm::drool:")
            return
        del db[server.id]
        fileIO(self.load, "save", db)
        await self.bot.say("Successfully deleted all settings for interview for this server.")
